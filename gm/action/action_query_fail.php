...
<?php
	...
	if (isset($_POST['action']) && $_POST['action'] == 'save' && $authorized) {
		// DELETE action
		$DB->execute("DELETE FROM gm_action_query_fail WHERE id_action = ".$actionID);
		$i = 1;
		$j = 1;
		$assigned_i = 1;
		$assigned_j = 0;
		...
		while (isset($_POST['c_'.$i.'_1'])) {
			...
			while (isset($_POST['c_'.$i.'_'.$j])) {
				...
				if ($_POST['s_'.$i.'_'.$j] != null && $_POST['s_'.$i.'_'.$j] != '') {
					$assigned = true;
					$assigned_j++;
					$DB->execute("INSERT INTO gm_action_query_fail(condition_string, sql_string, index_query, index_subquery, id_action) VALUES('".str_replace("'","''",$_POST['c_'.$i.'_'.$j])."','".str_replace("'","''",$_POST['s_'.$i.'_'.$j])."',".$assigned_i.",".$assigned_j.",".$actionID.")");
				}
				// INSERT action
				//echo '</div>';
				$j = $j + 1;
...
<a style="text-decoration:none; color:black; padding-right:215px;" href="action_query_success.php?id=<?php echo $actionID ?>"><img src="/gm/img/backward.png" title="Back to Query Set on Success"/>&nbsp;&nbsp;Back to Query Set on Success</a>
<span class="addable" style="vertical-align:top; cursor:pointer; padding-right:215px;"><img src="/gm/img/add.png" title="Add query"/>&nbsp;&nbsp;Add query</span>
<a style="text-decoration:none; color:black;" href="action_next_page.php?id=<?php echo $actionID ?>">Define this action next page&nbsp;&nbsp;<img src="/gm/img/forward.png" title="Define this action next page"/></a>
</div>
<form id="form" action="action_query_fail.php" method="post">
...
<?php if ($authorized) { ?>
	<button id="action" name="action" type="submit" value="save">Save this action query set on fail</button>
<?php } ?>
</form>
...
<script>
	...
	function deleteQueryPiece(e){
		...
		if (cancelConfirm == true) {
			if (id != 'new') {
				// css
				$("#titlepage").html('Action <i style="background-color:yellow;">"<?php echo $actionName?>"</i> Query Set on Fail <span style="color:red">changed</span>');
