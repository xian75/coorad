...
<div><a href="../index.php">Home</a> &gt; <a href="../application/?appId=<?php echo $appID; ?>">Configuration</a> &gt; <a href="index.php?appId=<?php echo $appID; ?>">Actions</a> &gt; Import</div>
...
<?php
		...
		if (isset($_POST['xmlfilename']) && $authorized) {
			...
			foreach ($nodes as $node) {
				$newname = importActionFromXml($DB, $node, $appID, $timestamp, $reuse);
			}
?>
			<div>XML has been imported correctly. The imported (or reused) action name is: <?php echo '<i style="background-color:yellow;">"'.str_replace('[REUSED]', '<span style="background-color:orange;">[REUSED]</span>', $newname).'"</i>' ?></div>
			<form action="index.php" method='post'>
				<input type="hidden" id="appId" name="appId" value="<?php echo $appID; ?>" />
				<input type="submit" id="back" name="back" value="Back to Actions" />
			</form>
<?php
		}
		else if ($authorized) {
			...
			if ($xmlerror == '') {
				...
				if (!isXmlActionCompliant($xmlfile)) $xmlerror = 'Bad XML file.';
			}
			if ($xmlerror == '') {
...
?>
				<form action="action_xml_import.php" method='post'>
