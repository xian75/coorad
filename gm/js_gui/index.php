		...
		if (isset($_POST['action']) && $_POST['action'] == 'save' && $authorized) {
			...
			while (isset($_POST['i_'.$i.'_1'])) {
				...
				while (isset($_POST['i_'.$i.'_'.$j])) {
					...
					if ($_POST['i_'.$i.'_'.$j] > 0) {
						// UPDATE javascript
						$DB->execute("UPDATE gm_javascript SET name='".str_replace("'","''",$_POST['n_'.$i.'_'.$j])."', description='".str_replace("'","''",$_POST['d_'.$i.'_'.$j])."' WHERE id=".$_POST['i_'.$i.'_'.$j]);
					}
					else {
						// INSERT javascript
						$DB->execute("INSERT INTO gm_javascript(name, description, id_application) VALUES('".str_replace("'","''",$_POST['n_'.$i.'_'.$j])."','".str_replace("'","''",$_POST['d_'.$i.'_'.$j])."',".$appID.")");
					}
					//echo '</div>';
					$j = $j + 1;
				}
			...
			if (isset($_POST['idstodelete'])) {
				...
				$DB->execute("DELETE FROM gm_javascript WHERE id IN ".$idstodelete);
				//echo "DELETE FROM gm_javascript WHERE id IN ".$idstodelete;
			}
		}
		if (isset($_GET['duplicateid']) && $authorized) {
			$javascriptnameArrayForDuplication = array();
			$res = $DB->execute("SELECT name FROM gm_javascript WHERE id_application = ".$appID);
			if($res){
				while ($row = $res->fetch_object()){
					$javascriptnameArrayForDuplication[] = $row->name;
				}
				$res->close();
			}
			$duplicate_id = $_GET['duplicateid'];
			$res = $DB->execute("SELECT name, description, javascript, id_application FROM gm_javascript WHERE id = ".$duplicate_id);
			if ($res) {
				$duplicate_sql = "INSERT INTO gm_javascript(name, description, javascript, id_application) VALUES(";
				if ($row = $res->fetch_object()) {
					$duplicate_name = $row->name;
					$r = 0;
					while (in_array($duplicate_name, $javascriptnameArrayForDuplication)) {
						$r++;
						$duplicate_name = $row->name."_copy".$r;
					}
					$duplicate_sql .= "'".str_replace("'","''",$duplicate_name)."',";
					$duplicate_sql .= "'".str_replace("'","''",$row->description)."',";
					$duplicate_sql .= "'".str_replace("'","''",$row->javascript)."',";
...
?>
<div><a href="../index.php">Home</a> &gt; <a href="../application/?appId=<?php echo $appID; ?>">Configuration</a> &gt; JavaScript</div>
<h3 id="subtitlepage">Application <?php if ($appID != 0) echo '<i style="background-color:yellow;">"'.$appName.'"</i>'; ?></h3>
<h2 id="titlepage">JavaScript</h2>
<form id="form" action="index.php" method="post">
	<div id="queries" style="border: dotted 1px; width:920px;">

		<div id="div_1" style="width:910px; background-color:#dddddd; padding-top:2px; padding-left:5px; padding-right:5px;" class="querysortable"><img src="/gm/img/add.png" style="vertical-align:top; cursor:pointer;" class="piece_addable" title="Add javascript"/><span class="piece_addable" style="cursor:pointer;">&nbsp;&nbsp;Add javascript</span>
			<ul id="query_1" class="sortable" style="width:910px">

<?php
		$res = $DB->execute("SELECT id, name, description FROM gm_javascript WHERE id_application = ".$appID." ORDER BY name ASC");
		if($res){
			$r = 0;
			while ($row = $res->fetch_object()){
				$r++;
?>
			  <li style="background-color:#eeeeee; margin-bottom:1px;" title="<?php echo $row->id?>">
				...
				<img src="/gm/img/del.png" style="vertical-align:middle; cursor:pointer;" class="piece_deletable" title="Cancel this javascript (id=<?php echo $row->id?>)"/>
<?php if ($authorized) { ?>
				<a href="index.php?appId=<?php echo $appID; ?>&duplicateid=<?php echo $row->id?>" onClick="return confirm('Do you really want to duplicate this javascript (id=<?php echo $row->id?>)?\nAll other chenges will be lost.');"><img src="/gm/img/duplicate.png" style="vertical-align:middle; cursor:pointer;" title="Duplicate this javascript (id=<?php echo $row->id?>)"/></a>
<?php } ?>
				<a href="javascript.php?id=<?php echo $row->id?>"><img src="/gm/img/forward.png" style="vertical-align:middle; cursor:pointer;" title="Select this javascript (id=<?php echo $row->id?>) to define its script"/></a>
			  </li>
<?php
			}
			$res->close();
...
<?php if ($authorized) { ?>
	<button id="action" name="action" type="submit" value="save">Save javascript</button>
<?php } ?>
</form>
...
<script>
	...
	function deleteQueryPiece(e){
		...
		var cancelConfirm = '';
		if (id != 'new') cancelConfirm = confirm('Do you really want to cancel this javascript (id=' + id + ')?\nRemember to submit this form to remove it definitely.');
		else cancelConfirm = confirm('Do you really want to cancel this new javascript?\nRemember to submit this form to remove it definitely.');
		if (cancelConfirm == true) {
			if (id != 'new') {
				$("#idstodelete").attr('value', id + ',' + $("#idstodelete").attr('value'));
				// javascript
				$("#titlepage").html('Javascript <span style="color:red">changed</span>');
