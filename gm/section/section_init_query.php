...
<?php
	...
	if (isset($_POST['action']) && $_POST['action'] == 'save' && $authorized) {
		// DELETE section
		$DB->execute("DELETE FROM gm_section_init_query WHERE id_section = ".$sectionID);
		$i = 1;
		$j = 1;
		$assigned_i = 1;
		$assigned_j = 0;
		...
		while (isset($_POST['c_'.$i.'_1'])) {
			...
			while (isset($_POST['c_'.$i.'_'.$j])) {
				...
				if ($_POST['s_'.$i.'_'.$j] != null && $_POST['s_'.$i.'_'.$j] != '') {
					$assigned = true;
					$assigned_j++;
					$DB->execute("INSERT INTO gm_section_init_query(condition_string, sql_string, index_query, index_subquery, id_section) VALUES('".str_replace("'","''",$_POST['c_'.$i.'_'.$j])."','".str_replace("'","''",$_POST['s_'.$i.'_'.$j])."',".$assigned_i.",".$assigned_j.",".$sectionID.")");
				}
				// INSERT section
				//echo '</div>';
				$j = $j + 1;
...
<a style="text-decoration:none; color:black; padding-right:228px;" href="section_pre_init_query.php?id=<?php echo $sectionID?>"><img src="/gm/img/backward.png" title="Back to Pre Init Query Set"/>&nbsp;&nbsp;Back to Pre Init Query Set</a>
<span class="addable" style="vertical-align:top; cursor:pointer; padding-right:228px;"><img src="/gm/img/add.png" title="Add query"/>&nbsp;&nbsp;Add query</span>
<a style="text-decoration:none; color:black;" href="section_view_selection.php?id=<?php echo $sectionID ?>">Define this section view set&nbsp;&nbsp;<img src="/gm/img/forward.png" title="Define this section view set"/></a>
</div>
<form id="form" action="section_init_query.php" method="post">
...
<?php if ($authorized) { ?>
	<button id="action" name="action" type="submit" value="save">Save this section initialization query set</button>
<?php } ?>
</form>
...
<script>
	...
	function deleteQueryPiece(e){
		...
		if (cancelConfirm == true) {
			if (id != 'new') {
				// css
				$("#titlepage").html('Section <i style="background-color:yellow;">"<?php echo $sectionName?>"</i> Initialization Query Set <span style="color:red">changed</span>');
